 ...
 *
 * <p>Handles all incoming request validation including ICAO code format, count limits, and
...
public class RequestValidator {
  ...
  public void validateAirportDetailsRequest(List<String> icaoCodes) {
    ...
    if (icaoCodes.size() > MAX_ICAO_CODES) {
      log.error(
          "Validation failed — {} ICAO codes exceeds limit of {}",
          icaoCodes.size(),
          MAX_ICAO_CODES);
      throw new AeroLinkException(
          ErrorCode.ICAO_LIMIT_EXCEEDED,
  ...
  /**
   * Validates that each ICAO code is in the correct format. Format: Exactly 4 letters
   *
   * @param icaoCodes list of ICAO codes to validate
  ...
  private void validateIcaoCodeFormat(List<String> icaoCodes) {
    for (String code : icaoCodes) {
      if (code == null || !ICAO_CODE_PATTERN.matcher(code).matches()) {
        ...
        throw new AeroLinkException(
            ErrorCode.ICAO_CODE_INVALID_FORMAT,
